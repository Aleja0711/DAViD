...
var option = document.getElementsByClassName("single-tab");

for ( var a = 0; a < option.length; a++){
  option[a].onclick = function(){
    var select =this.innerHTML;
    document.getElementById("box-text").innerHTML=(aboutUs[select]);    
    for (var b = 0; b < option.length; b++){
        option[b].style['background'] = unseletectedColor;
        option[b].style['font-weight'] ="normal";
    }
    this.style['background'] = seletectedColor;
...
document.getElementById("service-next").onclick = function(){
  document.getElementById("service-title").innerHTML = (ourServices[ejem]['title']);
  document.getElementById("service-text").innerHTML = (ourServices[ejem]['text']);
  if (ejem == ourServices.length-1){
    ejem=0;
  }else{
    ejem+=1;
  }
...
document.getElementById("current_year").innerHTML=(yearObj.getFullYear());
